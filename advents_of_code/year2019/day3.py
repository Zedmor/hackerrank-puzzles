...
def parse(route):
    ...
    groups = re.findall(r'(\w\d+)+', route)

    final_groups = [(dir_dict[c[0]], int(c[1:])) for c in groups]
...
def main():
    ...
    grid = np.zeros((50000, 50000))
    for num_w, commands in enumerate(wires, 1):
        parsed_commands = parse(commands)
        cursor = start
        total_steps = 0
        for command in parsed_commands:
            for _ in range(command[1] if cursor == start else command[1]):
                steps[cursor].append(total_steps)
                if grid[cursor] == 1 and num_w == 2:
                    grid[cursor] = 9
                else:
                    grid[cursor] = num_w
                cursor = tuple([sum(x) for x in zip(cursor,command[0])])
                total_steps += 1

    ii = np.argwhere(grid == 9)

    man_dist = lambda a, b: abs(a[0] - b[0]) + abs(a[1] - b[1])

    distances = [man_dist(coord, start) for coord in ii if man_dist(coord, start) != 0]

    for coord in ii:
        print(steps[tuple(coord)])
        print(sum(steps[tuple(coord)]))
