...
inp = """striped green bags contain 5 posh indigo bags.
...
for line in inp.split('\n'):
    container, bags = re.findall('(.+) bags contain (.+)', line)[0]
    for subbag in bags.split(','):
        if subbag == 'no other bags.':
            continue
        n, name = re.findall('(\d) (.+) bag', subbag)[0]

        g[container].append((int(n), name))
