...
for line in src.split('\n'):
    ...
    if not line:
        continue
    groups = re.findall('(\d+)\-(\d+)\s(\w):\s(\w+)', line)[0]
    low, high, letter, passw = groups
    cn = 0
    if letter in passw[int(low) - 1]:
        cn += 1

    if letter in passw[int(high) - 1]:
        cn += 1

    if cn == 1:
        counter += 1
    # c = Counter(passw)
    # if letter in c and int(low) <= c[letter] <= int(high):
    #     counter += 1
