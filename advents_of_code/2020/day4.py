...
inp = """eyr:2028 iyr:2016 byr:1995 ecl:oth
...
def ecl_check(v):
    checks = {'amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'}
...
def validate(c):
    ...
    for l in c:
        for i, f in enumerate(fields):
            res = re.findall(f'{f}:(\S+)', l)
            if res and field_validators[f](res[0]):
                check[i] = True
    check[7] = True
    # if not all(check):
    #     print(dict(zip(fields, check)), c)
    return all(check)
...
def main():
    ...
    for l in inp.split('\n'):
        if not l:
            counter += validate(coll)
            coll = []
        else:
            coll.append(l)

    counter += validate(' '.join(coll))
