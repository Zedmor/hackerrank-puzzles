...
class Solution:
    ...
    >>> Solution().getKth(lo = 12, hi = 15, k = 2)
    13

    >>> Solution().getKth(lo = 1, hi = 1, k = 1)
    1
    ...
    def getKth(self, lo: int, hi: int, k: int) -> int:
        ...
        @lru_cache(None)
        def recur(key, depth=1):
            if key == 1:
                return depth
            if key % 2 == 0:
                return recur(key // 2, depth + 1)
            else:
                return recur(key * 3 + 1, depth + 1)

        results = []
        for i in range(lo, hi + 1):
            results.append((recur(i), i))
