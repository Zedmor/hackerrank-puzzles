"""Given a string of numbers and operators, return all possible results from
computing all the different possible ways to group numbers and operators. The
valid operators are +, - and *.


Example 1
Input: "2-1-1".

((2-1)-1) = 0
(2-(1-1)) = 2
Output: [0, 2]


Example 2
Input: "2*3-4*5"

(2*(3-(4*5))) = -34
((2*3)-(4*5)) = -14
((2*(3-4))*5) = -10
(2*((3-4)*5)) = -10
(((2*3)-4)*5) = 10
Output: [-34, -14, -10, -10, 10]
"""





class Solution(object):
    def resop(self, num1, num2, operator):
        num1 = int(num1)
        num2 = int(num2)
        if operator == '+':
            return str(num1 + num2)
        if operator == '-':
            return str(num1 - num2)
        if operator == '*':
            return str(num1 * num2)
    def diffWaysToCompute(self, input):
        """
        :type input: str
        :rtype: List[int]
        """
        import re
        import itertools
        numbers = re.findall(r'\d+', input)
        operators = re.findall(r'\+|\-|\*+', input)
        # print(numbers, operators)
        totalresult = []
        for order in (itertools.permutations(range(len(operators)))):
            print(order)
            numbackup = numbers[:]
            operbackup = operators[:]
            listorder = list(order)
            while listorder:
                operation = listorder.pop()
                replacement = self.resop(numbackup[operation],
                                    numbackup[operation + 1],
                                    operbackup[operation])
                # stack.append(numbackup[operation])
                # stack.append(operbackup[operation])
                # stack.append(numbers[operation+1])

                numbackup[operation] = replacement
                numbackup.pop(operation+1)
                operbackup.pop(operation)
                for position, item in enumerate(listorder):
                    if item > operation:
                        listorder[position] -=1

                # = replacement

            totalresult.append(int(numbackup[0]))
        return totalresult

inputstring = "2*3-4*5-2+1"
a = Solution()
print(a.diffWaysToCompute(inputstring))
