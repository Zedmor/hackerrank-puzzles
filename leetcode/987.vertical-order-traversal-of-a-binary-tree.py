...
class Solution:
    """
    >>> Solution().verticalTraversal(list2tree('[0,5,1,9,null,2,null,null,null,null,3,4,8,6,null,null,null,7]'))
    [[[9,7],[5,6],[0,2,4],[1,3],[8]]

    >>> Solution().verticalTraversal(list2tree('[3,9,20,null,null,15,7]'))
    [[9],[3,15],[20],[7]]
    ...
    def verticalTraversal(self, root: TreeNode) -> List[List[int]]:
        ...
        def recur(node, offset=0, depth=0):
            orders[offset].append((depth, node.val))
            if node.left:
                recur(node.left, offset - 1, depth + 1)
            if node.right:
                recur(node.right, offset + 1, depth + 1)

        recur(root)
        result = []
        for k in sorted(orders.keys()):
            result.append([t[1] for t in sorted(orders[k])])
        return result
