...
class Solution:
    """
    >>> Solution().rankTeams(["AXYB","AYXB","AXYB","AYXB"])
    'AXYB'

    >>> Solution().rankTeams(["ABC","ACB","ABC","ACB","ACB"])
    'ACB'

    >>> Solution().rankTeams(["WXYZ","XYZW"])
    'XWYZ'

    >>> Solution().rankTeams(["BCA","CAB","CBA","ABC","ACB","BAC"])
    'ABC'

    >>> Solution().rankTeams(["M","M","M","M"])
    'M'

    """

    def rankTeams(self, votes: List[str]) -> str:
        rank = [[0 for x in range(len(votes[0]))] for y in range(len(votes[0]))]
        teams = sorted(votes[0])
        for n, round in enumerate(zip(*votes)):
            counts = Counter(round)
            for i, t in enumerate(teams):
                rank[i][n] = counts.get(t, 0)
        els = [(r, t) for r, t in zip(rank, teams)]
        res = sorted(els, key=lambda t: (t[0], ta- ord(t[1])), reverse=True)
