...
class Solution(object):
    def findWords(self, board, words):
        ...
        def findwordfrompos(word, root, found=False, visited=[], index=0):
            if found:
                return found
            y, x = root
            if x >= 0 and x < maxX and y >= 0 and y < maxY and root not in visited:
                letter = board[y][x]
                newlist = [wrd for wrd in words if len(wrd) > index and wrd[index]==letter]
                if newlist:
                    # print(board[root[0]][root[1]])
                    newlist = [wrd for wrd in newlist if
                               len(wrd) > index and wrd[index:] == letter]
                    if newlist:
                        if not found:
                            found = newlist
                        else:
                    ...
                    found = findwordfrompos(word, (y, x - 1), found,
                                            visited + [root], index+1)
                    found = findwordfrompos(word, (y, x + 1), found,
                                            visited + [root], index+1)
                    found = findwordfrompos(word, (y - 1, x), found,
                                            visited + [root], index+1)
                    found = findwordfrompos(word, (y + 1, x), found,
                                            visited + [root], index+1)
            if found:
            ...
            for x in range(maxX):
                for y in range(maxY):
                    if board[y][x] == word[0]:
                        # visited.append((x,y))
                        root = (y, x)
                        foundword = findwordfrompos(word, root)

                        if word not in solution and foundword:
                                foundword = list(set(foundword))
                                solution = solution + foundword
                                indices = [i for i, x in enumerate(words) if
                                           x in foundword]
...
a = Solution()
print(a.findWords(["oaan","etae","ihkr","iflv"],["oath", "pea", "eat", 'eat', "rain"] ))

print(a.findWords(
    ["aaaa", "aaaa", "aaaa", "aaaa", "bcde", "fghi", "jklm", "nopq", "rstu",
     "vwxy", "zzzz"],
